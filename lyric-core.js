...
const GENRE_PRESETS={
  trap:{x:20,y:58,grid:"triplet"}, drill:{x:35,y:58,grid:"triplet"}, boombap:{x:25,y:22,grid:"straight"},
  cloud:{x:80,y:62,grid:"straight"}, emo:{x:58,y:25,grid:"straight"}, melodic:{x:72,y:28,grid:"straight"}, poprap:{x:48,y:22,grid:"straight"}
};
function applyGenrePreset(g){const p=GENRE_PRESETS[g];if(!p)return;
  feelX=p.x;feelY=p.y;gridMode=p.grid||"straight";
  try{localStorage.setItem(FEELX,feelX);localStorage.setItem(FEELY,feelY);localStorage.setItem(GRIDK,gridMode);}catch(e){}
  renderFeel();renderGrid();}
if($("genrePreset"))$("genrePreset").onchange=()=>applyGenrePreset($("genrePreset").value);
/* GRID TYPE — straight (duple 8th/16th) vs triplet (trap/drill: 3 per beat, denser, rolling).
   The concrete handle for the "packed" side of the table + the lever that lets genres diverge. */
const GRIDK="ams.lyrics.grid";
let gridMode=localStorage.getItem(GRIDK)||"straight";
...
6. VARY THE RHYME ACROSS THESE BARS: move through SEVERAL end-vowels — a new rhyme sound roughly every couplet (2 bars). Do NOT end more than 2 bars in a row on the same vowel unless deliberately building a list. ${n>=4?`So ${n} bars should use ~${Math.max(2,Math.round(n/2))} different end-vowels, not one.`:""} Do NOT default to the long-I / "AY" sound (time/night/sky/why/ride) — that is the lazy go-to; pick fresh, varied vowels. (A run of ≥8 bars on one vowel reads as monotonous — never do that.)
